...
function Videoitam({ sources }) {
  ...
  return (
    <Fragment>
      <video
        ref={video_ref}
        style={{ height: "100px", weigth: "100px", margin: "10px" }}
        src={sources}
        //   onMouseOver={(event) => {
      ...
      <div>
        <button
          onClick={() => {
            ...
            // console.log(video_ref.current.play());
          }}
        >
          Play
        </button>
...
function App() {
  const videoArray = mediaJSON.categories[0].videos;
    ...
    <div>
      <div>
        ...
        {videoArray.map((item, index) => {
          return <Videoitam sources={item.sources} key={index} />;
        })}
      </div>
    </div>
  );
